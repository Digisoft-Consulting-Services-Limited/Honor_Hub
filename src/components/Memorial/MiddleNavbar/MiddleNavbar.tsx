      ...
      {isMobileView ? (
        activeButton === "LIFE" ? (
          ...
          <div className="flex overflow-x-auto p-4 gap-2">
            {["LIFE", "PROGRAM", "TRIBUTES", "HYMNS", "ALBUM"].map((item) => (
              <button
                key={item}
                onClick={() => setActiveButton(item)}
                className={`p-2 rounded-md font-medium transition-all duration-200 flex items-center gap-2 whitespace-nowrap ${
                  activeButton === item
          ...
          // Show back button and image when ALBUM is active
        ...
        <div className="flex justify-center space-x-8 py-3">
          {["LIFE", "PROGRAM", "TRIBUTES", "HYMNS", "ALBUM"].map((item) => (
            <button
              key={item}
              onClick={() => setActiveButton(item)}
              className={`p-2 rounded-md font-medium transition-all duration-200 flex items-center gap-2 ${
                activeButton === item
                  ? "text-primary bg-primary-hover_light"
